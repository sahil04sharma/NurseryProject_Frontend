...
const AddAddress = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.isDefault) payload.append("isDefault", formData.isDefault);
  ...
  const getPostalCode = async (pin) => {
    ...
    try {
      ...
      if (data.city && data.state) {
        ...
        console.warn("[AddAddress] Invalid PIN payload:", data);
  ...
  return (
    <div className="flex py-6 justify-center items-center min-h-screen pt-26 sm:pt-8 md:pt-0 bg-gray-100">
      <form
        onSubmit={handleSubmit}
        className="bg-white shadow-lg rounded-2xl px-6 py-2 w-full max-w-4xl"
      >
        <h2 className="text-xl relative md:text-2xl font-bold text-gray-800 mb-4 sm:mb-6 text-center">
          <button
            type="button"
            onClick={() => navigate(-1)}
        ...
        <div className="flex justify-center mb-4 sm:mb-6">
          <button
            ...
            className="bg-green-600 text-xs hover:bg-green-700 text-white px-4 py-2 rounded-lg flex items-center gap-2"
          >
            {loadingLocation ? (
        ...
        <button
          type="submit"
          disabled={loading}
          className={`mt-6 w-full bg-linear-to-r from-green-500 to-green-700 hover:shadow-lg text-white py-2 rounded-lg ${
